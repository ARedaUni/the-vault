 ...
 * merge. Both projects inherit it via `extends: true`.
 */
 ...
 *
 * `browser` runs components and the app itself in real Chromium, with MSW
 * answering the network. This is where app behaviour per API response belongs —
 * loading, failure, retry, empty — because none of it needs a dev server, a
 * navigation or a real media fetch. Playwright is reserved for the handful of
 * journeys that do.
 *
